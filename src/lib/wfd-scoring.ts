/**
 * PTE Write From Dictation (WFD) scoring engine.
 *
 * Deterministic on purpose — WFD is exact word matching, not judgement, so it
 * runs as plain code: instant, free, and identical every time. (LLMs are
 * unreliable at exact alignment and counting.)
 */
...
export interface WfdResult {
  ...
  pteScore: number;
  feedback: string[];
  /** Word-by-word breakdown for the UI. */
  analysis: WfdToken[];
}
...
export interface WfdToken {
  kind: WfdTokenKind;
  /** Word from the official transcript (absent for `extra`). */
  expected?: string;
  /** Word the student wrote (absent for `missing`). */
  actual?: string;
}
...
/** Levenshtein distance — used to tell a misspelling from a different word. */
...
/**
 * A substitution counts as a misspelling (rather than a different word) when
 * the two are close enough: within edit distance 2, and no more than a third
 * of the longer word.
 */
...
/** Needleman–Wunsch alignment so words line up even with inserts/deletes. */
function align(official: string[], student: string[]): { op: Op; o?: string; s?: string }[] {
  ...
  for (let i = 1; i <= n; i++) {
    for (let j = 1; j <= m; j++) {
      ...
      // Prefer aligning near-identical words (misspellings) over del+ins.
...
/**
 * Word-order errors: words the student wrote that DO exist in the transcript
 * but landed in the wrong place (so they show as a delete + insert pair).
 */
function detectOrderErrors(
  ...
  // Case 1 — a word was dropped here and re-appears elsewhere as an insert.
  ...
  // Case 2 — two adjacent words swapped. Alignment renders this as a pair of
  // substitutions that mirror each other (A→B and B→A), not delete+insert.
  for (const i of subIdx) {
    if (transposed.has(i)) continue;
    const a = ops[i];
    const j = subIdx.find(
      k => k > i && !transposed.has(k) && ops[k].o === a.s && ops[k].s === a.o
    );
    if (j !== undefined) {
      transposed.add(i);
      transposed.add(j);
...
/** Score a student's dictation attempt against the official transcript. */
export function scoreWfd(officialTranscript: string, studentAnswer: string): WfdResult {
  ...
  ops.forEach((op, idx) => {
    switch (op.op) {
      case 'match':
        ...
        break;
      case 'sub':
        // A mirrored substitution pair is a swap, not two wrong words.
        ...
        break;
      case 'del':
        // A displaced word is an order error, not a missing word.
  ...
  return {
    ...
    feedback: buildFeedback({
      missingWords, incorrectWords, misspelledWords, extraWords,
      orderErrors, accuracy, correctWords, totalWords,
    }),
    analysis,
  };
}
...
/** One-line performance summary for the results header. */

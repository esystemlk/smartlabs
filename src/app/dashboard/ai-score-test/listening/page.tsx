...
const listeningTasks = [
    {
        ...
        description: 'Listen to a short lecture and write a 50-70 word summary in 10 minutes.',
        icon: Mic2,
        difficulty: 'Hard',
        time: '10 mins',
        count: 5,
        ...
        <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="h-14 w-14 rounded-[2rem] bg-accent-4/10 flex items-center justify-center border border-accent-4/20 shadow-inner">
                            <Headphones className="h-7 w-7 text-accent-4" />
                        </div>
                        <div>
                            <h1 className="text-4xl font-black font-display tracking-tight">Listening Matrix</h1>
                ...
                <div className="flex gap-2">
                    <div className="glass-card px-6 py-3 rounded-2xl flex flex-col items-center">
                        ...
                        <span className="text-xl font-black text-accent-4">76</span>
                    </div>
                    <div className="glass-card px-6 py-3 rounded-2xl flex flex-col items-center">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-4">
                    <h2 className="text-sm font-black uppercase tracking-[0.2em] text-muted-foreground ml-2 mb-4">Select Acoustic Architecture</h2>
                        ...
                        {selectedTask ? (
                            ...
                            >
                                <div className="flex items-center justify-between px-2">
                                    <h2 className="text-xl font-black font-display tracking-tight flex items-center gap-2">
                                        <Zap className="h-5 w-5 text-primary" /> Questions for {listeningTasks.find(t => t.id === selectedTask)?.title}
                                    </h2>
                                </div>
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {[1, 2, 3, 4, 5].map((qNum) => (
                                        <Link
                                            key={qNum}
                                            href={`/dashboard/ai-score-test/listening/${selectedTask}/${qNum}`}
                                            className="block"
                                        >
                                            <SpotlightCard className="p-6 rounded-[2rem] border-2 bg-card/40 backdrop-blur-md hover:border-primary/50 transition-all group flex items-center justify-between">
                                                <div className="flex items-center gap-6">
                                                    ...
                                                    <div>
                                                        <h4 className="font-black text-lg group-hover:text-primary transition-colors">Acoustic Mastery Set #{qNum}</h4>
                                                        <div className="flex items-center gap-4 mt-1">
                                                            <span className="text-[10px] font-bold text-muted-foreground uppercase flex items-center gap-1">
                                                                <Clock className="h-3 w-3" /> Real Exam Audio
                                                            </span>
                            ...
                            <div className="h-full min-h-[500px] border-2 border-dashed border-border/50 rounded-[3rem] flex flex-col items-center justify-center p-12 text-center bg-muted/20">
                                <div className="h-24 w-24 rounded-[3rem] bg-background shadow-inner flex items-center justify-center mb-8 border border-border/50">
                                    <Headphones className="h-10 w-10 text-muted-foreground/50 animate-pulse" />
                                </div>
                                <h3 className="text-2xl font-black mb-2">Ready to Listen?</h3>
                                <p className="text-muted-foreground font-medium max-w-sm">Select a task on the left to initialize the AI acoustic scoring matrix and begin your session.</p>
                            </div>
                        )}
                    </AnimatePresence>
                </div>

...
import { testResultService } from '@/lib/services/test-results.service';
        ...
        try {
            ...
            if (user) {
                await testResultService.saveTestResult({
                    userId: user.uid,
                    testId: 'pte-reading-test-1',
                    testType: 'PTE Academic',
                    category: 'Reading',
                    score: Math.round(scoreResult.overallScore * 0.9), // Normalizing to 90 scale if needed
                    maxScore: 90,
                });

                await logTestCompletion(
                    user.uid,
                    'PTE Reading Practice Complete',
                    Math.round(scoreResult.overallScore * 0.9),

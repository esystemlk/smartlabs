...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!merchantSecret) { console.error('[mock-notify] secret missing'); return new Response('OK', { status: 200 }); }
    ...
    if (localSig !== String(md5sig).toUpperCase()) {
      console.warn(`[mock-notify] MD5 mismatch for order ${order_id}`);
    ...
    const ordersSnap = await adminDb
      ...
      .where('type', '==', 'mock_credits')
      .limit(1).get();
    if (ordersSnap.empty) { console.warn(`[mock-notify] order ${order_id} not found`); return new Response('OK', { status: 200 }); }
    ...
    if (sc === '2') {
      ...
      await adminDb.runTransaction(async tx => {
        ...
        tx.update(userRef, { mockPaidCredits: FieldValue.increment(mockCredits) });
      });
      console.log(`[mock-notify] ✅ ${mockCredits} mock credit(s) granted — user=${userId}`);
    } else if (sc === '-1') {

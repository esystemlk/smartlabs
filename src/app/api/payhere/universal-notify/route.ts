...
/**
 * PayHere webhook for UNIVERSAL AI credits. Grants `universalPaidCredits` (usable
 * by every AI-scored part), or a 40-day `universalMonthlyExpiry` for the
 * Unlimited package.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (secrets.length === 0) { console.error('[universal-notify] secret missing'); return new Response('OK', { status: 200 }); }
    ...
    if (!secrets.some((sec) => md5(base + md5(sec)) === want)) {
      console.warn(`[universal-notify] MD5 mismatch for order ${order_id}`);
    ...
    const ordersSnap = await adminDb
      ...
      .where('type', '==', 'universal_credits')
      .limit(1).get();
    if (ordersSnap.empty) { console.warn(`[universal-notify] order ${order_id} not found`); return new Response('OK', { status: 200 }); }
    ...
    if (sc === '2') {
      ...
      if (packageId === 'universal_unlimited') {
        const expiry = new Date();
        expiry.setDate(expiry.getDate() + 40);
        userUpdates.universalMonthlyExpiry = expiry;
      } else {
        userUpdates.universalPaidCredits = FieldValue.increment(scoringCredits);
      }
      ...
      console.log(`[universal-notify] ✅ universal credits granted — user=${userId} pkg=${packageId}`);
    } else if (sc === '-1') {
    ...
    console.error('[universal-notify] error:', error);
    return new Response('OK', { status: 200 });
  }
}

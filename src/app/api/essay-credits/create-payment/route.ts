...
export const ESSAY_PACKAGES = [
  ...
  { id: 'essay_20',        scoring: 20,  gens: 5,   price: 2000,  label: '20 Tests' },
  { id: 'essay_30',        scoring: 30,  gens: 8,   price: 2500,  label: '30 Tests' },
  { id: 'essay_40',        scoring: 40,  gens: 10,  price: 3500,  label: '40 Tests' },
  { id: 'essay_100',       scoring: 100, gens: 25,  price: 6000,  label: '100 Tests' },
  { id: 'essay_unlimited', scoring: -1,  gens: 300, price: 15000, label: 'Unlimited' },
] as const;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pkg = ESSAY_PACKAGES.find(p => p.id === packageId);
    if (!pkg) {
    ...
    const params = {
      ...
      return_url:   `${appUrl}/ai-essay-practice?payment=success&pkg=${pkg.id}`,
      cancel_url:   `${appUrl}/ai-essay-practice?payment=cancelled`,
      notify_url:   `${appUrl}/api/payhere/essay-notify`,
      order_id:     orderId,
      items:        `SmartLabs PTE Essay Credits — ${pkg.label}`,
      amount,
      currency,
    ...
    console.error('[essay-credits/create-payment]', error);
